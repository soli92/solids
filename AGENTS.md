...
Pacchetto **`@soli92/solids`**, Node **22+**, build in `dist/`. Documentazione interattiva: **Storybook** (`npm run storybook`). Integrazione shadcn: `docs/shadcn-integration.md`, `docs/registry-model-1.md`.

## Cosa fare dopo (checklist)

1. **Modifiche a token/CSS** — `npm run build`; verificare Storybook dopo cambi UI.
...
`npm run build` · `npm run storybook` · `npm run build-storybook` · `npm run registry:sync` · `npm run registry:build`
