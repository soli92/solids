...
const main = () => {
  ...
  ensureDir(join(DIST, "brand-assets"));
  ...
  cpSync(
    join("docs", "brand-assets", "workspace-icons-normalized"),
    join(DIST, "brand-assets", "workspace-icons-normalized"),
    {
      recursive: true,
    }
  );
  ...
  const normalizedIndex = readJson(normalizedIndexPath);
  const normalizedCategories = Object.keys(normalizedIndex.categories ?? {});
  const normalizedFiles = normalizedCategories.flatMap(
    (category) => normalizedIndex.categories[category]?.files ?? []
  );
  const workspaceIconsIndexJs =
    `/* Auto-generated by scripts/build.mjs */\n` +
    `export const workspaceIconsIndex = ${JSON.stringify(normalizedIndex, null, 2)};\n` +
    `export default workspaceIconsIndex;\n`;
  writeFileSync(
    join(DIST, "brand-assets", "workspace-icons-normalized", "index.js"),
    workspaceIconsIndexJs,
    "utf8"
  );
  const categoryUnion =
    normalizedCategories.length > 0
      ? normalizedCategories.map((c) => `"${c}"`).join(" | ")
      : "never";
  const fileUnion =
    normalizedFiles.length > 0
      ? normalizedFiles.map((f) => `"${f}"`).join(" | ")
      : "never";
  const workspaceIconsIndexDts =
    `/* Auto-generated by scripts/build.mjs */\n` +
    `export type WorkspaceIconCategory = ${categoryUnion};\n` +
    `export type WorkspaceIconFile = ${fileUnion};\n` +
    `export interface WorkspaceIconsIndex {\n` +
    `  count: number;\n` +
    `  categories: Record<WorkspaceIconCategory, { count: number; files: WorkspaceIconFile[] }>;\n` +
    `}\n` +
    `export declare const workspaceIconsIndex: WorkspaceIconsIndex;\n` +
    `export default workspaceIconsIndex;\n`;
  writeFileSync(
    join(DIST, "brand-assets", "workspace-icons-normalized", "index.d.ts"),
    workspaceIconsIndexDts,
    "utf8"
  );
